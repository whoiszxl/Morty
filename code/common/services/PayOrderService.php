...
class PayOrderService extends BaseService {
	...
	public static function createPayOrder( $member_id,$items = [],$params = []){
		...
		$total_price = 0;
        $continue_cnt = 0;
        //遍历传过来的商品判断价钱是否小于0了出错了continue_cnt就加1
        ...
        //开启事务了
        /**
         * 在sql后面加 count = 5的, 同时执行就会一条执行,另一条执行不满足,乐观锁,
         * select for update 悲观锁,在查询的时候直接锁住,不允许别人再去查询了,并发不高,使用这个
         */
		$connection =  PayOrder::getDb();
		$transaction = $connection->beginTransaction();
		try{
            //为了防止并发 库存出问题了，做下select for update
            //获取到表名
            ...
            //遍历,将库存信息保存到tmp_book_unit_mapping中
